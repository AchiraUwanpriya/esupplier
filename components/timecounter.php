<?php
include __DIR__ . '/../backend/common/config.php';
...
// $tsql = "SELECT mtd_start_date, mtd_end_date, mtd_bidclose_date, mtd_tender_no FROM  mms_tender_details WHERE mtd_type = '$user_category' ORDER BY mtd_bidclose_date DESC LIMIT 1 ";

if($user_category === 'RI') {
    // Return NULL values for RI category
    $tsql = "SELECT mtd_start_date, mtd_end_date, mtd_bidclose_date, mtd_tender_no FROM mms_tender_details WHERE mtd_type IS NULL ORDER BY mtd_bidclose_date DESC LIMIT 1";
} else {
    $tsql = "SELECT mtd_start_date, mtd_end_date, mtd_bidclose_date, mtd_tender_no FROM mms_tender_details WHERE mtd_type = '$user_category' ORDER BY mtd_bidclose_date DESC LIMIT 1";
}
