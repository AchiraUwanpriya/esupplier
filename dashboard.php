...
// 		if ($_POST['insert'] == 'vegitables' || 'spices' || 'fish' || 'dfish' || 'rice'|| 'pvcs' || 'med'||'cab') {
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($_POST['insert'])) {
        ...
        if (!in_array($_POST['insert'], $allowed, true)) {
            // not a category save request
        } else {

            // use logged user's category to pick the correct active tender (mtd_type = sup_category)
            $user_category = isset($_SESSION['sup_category']) ? $_SESSION['sup_category'] : '';
            if ($user_category === '') {
            ...
            // fetch the single active tender for this category
			...
			if ($user_category === 'RI') {
                ...
                $tnStmt = mysqli_prepare($con, "SELECT mtd_tender_no, mtd_year FROM mms_tender_details WHERE mtd_status = 'A' AND mtd_type IS NULL ");
            } else {
                // category-specific tenders (PI, MI, CB, etc.)
                $tnStmt = mysqli_prepare($con, "SELECT mtd_tender_no, mtd_year FROM mms_tender_details WHERE mtd_status = 'A' AND mtd_type = ? ");
                mysqli_stmt_bind_param($tnStmt, 's', $user_category);
            }
            mysqli_stmt_execute($tnStmt);
            mysqli_stmt_bind_result($tnStmt, $tenderNo, $tenderYear);
            ...
            if (!$fetched) {
                ...
                if (!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) === 'xmlhttprequest') {
                    ...
                    echo "<script>alert('No active tender found for your category');</script>";
                }
                exit;
            }
            ...
            // prepared statements for row operations
            ...
            for ($x = 0; $x < $rowCount; $x++) {
                ...
                if ($MMC_MATERIAL_CODE === '') continue; // skip malformed
            ...
            if ($deleteStmt) mysqli_stmt_close($deleteStmt);
            ...
            if (!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) === 'xmlhttprequest') {
                ...
                else echo "<script>alert('Error occurred while saving the data!');</script>";
            }
            return;
        }
    }
	...
	<script>
		$(document).ready(function() {
			...
			$(".next").click(function(e) {
				...
				// Get the parent fieldset - traverse up to find the correct fieldset
				current_fs = $(this).closest("fieldset");
				next_fs = current_fs.next();
				
				// Make sure we're not going beyond the last fieldset
				if (next_fs.length === 0) return;
				
				// Get the index of the next fieldset
				var next_index = $("fieldset").index(next_fs);
				
				// Add Class Active to the progress bar li for the next step
				...
				percent = percent.toFixed(0); // Round to whole number
				$(".progress-bar").css("width", percent + "%");
				
				// Update the current step variable if needed elsewhere
			...
			$(".previous").click(function(e) {
				...
				// Remove class active from current step
